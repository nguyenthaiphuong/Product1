  ...
  class SessionsController < DeviseTokenAuth::SessionsController
    ...
    def create
      ...
      if @resource and (params[:provider] == 'facebook' || (valid_params?(field, q_value) and @resource.valid_password?(resource_params[:password]) and (!@resource.respond_to?(:active_for_authentication?) or @resource.active_for_authentication?)))
        ...
        #users = User.find_by
        # users = ActiveModel::Serializer::CollectionSerializer.new( @resource.token_validation_response,
        ...
        #   @resource.token_validation_response, each_serializer: UserSerializer
      elsif @resource and not (!@resource.respond_to?(:active_for_authentication?) or @resource.active_for_authentication?)
        render_create_error_not_confirmed
      else
        #render_create_error_bad_credentials
        render json: {code: "500", message: "login fails"}
      end
    end
